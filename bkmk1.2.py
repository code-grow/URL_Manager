...
# Function to open the Python script file in Notepad
'''
def open_script_file_notepad():
    script_file_path = os.path.abspath(__file__)
    os.system(f'notepad "{script_file_path}"')  # Open script file with Notepad
'''
def open_json_file_notepad():
    folder_name = folder_entry.get()
    selected_item = tree_view.focus()
    selected_file = tree_view.item(selected_item, "text")
    if folder_name.strip() == "":
...
# Button to open script file in Notepad
open_script_button = tk.Button(folder_management_frame, text="Open Script in Notepad", command=open_json_file_notepad, bg=primary_color, fg="white")
open_script_button.grid(row=2, column=0, columnspan=5, pady=5)
...
show_structure_button.grid(row=3, column=0, columnspan=5, pady=5)

tree_view_frame = tk.Frame(folder_management_frame)  # Set background color for tree view
tree_view_frame.grid(row=4, column=0, columnspan=5, pady=5, sticky="nsew")
...
folder_management_frame.grid_rowconfigure(4, weight=1)
